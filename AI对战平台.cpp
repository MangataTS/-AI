...
const int M = 16;
int N = 8;
const int attack_key[6] ={0,1,40,200,999999,999999};//分别对应着的是这个方向落子个数的进攻价值 
const int defense_key[6] = {0,1,50,99999,9999999,9999999};//分别对应这是这个方向落子的防御价值 
int dx[8] = {0,-1,-1,-1,0,1,1,1};
int dy[8] = {-1,-1,0,1,1,1,0,-1};

int bord[M][M];
int bord_key[M][M];//整个棋盘的估值计算值
int own = 1;
...
int evaluate_attack(int x,int y,int c) { //进攻估值函数 
	...
	if((x == 1 || x == N - 1 || y == 1 || y == N - 1 ) && ans < 3) des = 9999999;
	
	return max(0,attack_key[min(ans,5)] - des);//返回一个计算(i,j)落点的价值，不考虑33，44，长连禁手等规则  
}
...
int evaluate_defense(int x,int y,int c) {
	...
	if(l && r) {
		int mid = l + r;
		if(mid == 2)  defens_sorce += 50;
		else if(mid == 3) defens_sorce += 9999;
		else if(mid >= 4) defens_sorce += 999999;
	}
	...
	int des = 0;
	//if((x == 1 || x == N - 1 || y == 1 || y == N - 1 ) && ans < 3) des = 9999999;
	return max(defense_key[min(ans,5)] - des,defens_sorce);
}
...
void PM()
{
    ...
    {
    	pair<int,int> ttt = DFS(own);
    	bord[ttt.first][ttt.second] = own;
    	p.x = ttt.first - 1;
    	p.y = ttt.second - 1;
        if(p.x<0)
        {
